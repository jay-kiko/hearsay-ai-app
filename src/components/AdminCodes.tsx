...
export function AdminCodes({ codes, onGenerate, onRevoke, onBack }: AdminCodesProps) {
  ...
  return (
    <div className="max-w-[780px] mx-auto px-7 py-12 pb-[110px] animate-fadeUp">
      ...
      <p className="text-[15px] text-[#888] mb-8">Generate invite codes and set how many times each can be used.</p>
      ...
      <div className="bg-white border border-[#ECECEC] rounded-[16px] p-[26px] mb-[18px] flex items-end gap-4 flex-wrap">
        ...
        <button
          ...
          className="bg-[#2D6AE0] text-white border-none rounded-[11px] px-5 py-3 text-sm font-semibold cursor-pointer hover:bg-[#2560d0] transition-colors"
        >
          + Generate code
        </button>
      </div>
        ...
        {codes.map((c, i) => {
          ...
          return (
            <div key={c.code} className={`grid grid-cols-[1.5fr_1fr_1fr_1fr] px-[22px] py-4 text-sm text-[#444] items-center ${i < codes.length - 1 ? 'border-b border-[#F4F4F4]' : ''}`}>
              ...
              <div className="text-[13px] text-[#888]">{c.createdAt}</div>
              <div className="flex items-center gap-3">
                <span
                  className="text-[11.5px] font-semibold rounded-full px-[11px] py-1"
                  style={{ color: exhausted ? '#A0A0A0' : '#1E9E6A', background: exhausted ? '#F0F0F0' : '#E8F6EF' }}
                >
                  {exhausted ? 'Exhausted' : 'Active'}
                </span>
                {!exhausted && (
                  <span onClick={() => onRevoke(c.code)} className="text-[12px] text-[#C2543A] cursor-pointer hover:opacity-80">Revoke</span>
                )}
              </div>
            </div>
          );
